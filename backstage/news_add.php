	...
	<body>
		<div class="x-body">
			<form class="layui-form" enctype="multipart/form-data" method="post">
				...
				<div class="layui-form-item">
					<label for="newType" class="layui-form-label">新闻类型</label>
					<div class="layui-input-inline">
						<select lay-filter="choiceNews">
				...
				<div id="rotationPhoto" class="layui-form-item" hidden="hidden">
		...
		<script>
			...
			layui.use(['form', 'layer'],function() {
                ...
                //新闻类型的判断，如果选择轮播新闻则隐藏封面图片的input。则反
				form.on('select(choiceNews)', function(data1){
				    var choiceNews = data1.value;
				    switch (choiceNews){
				    	case "generalNews":
				    		$('#coverPhoto').removeAttr("hidden");
				    		$('#rotationPhoto').attr("hidden","hidden");
				    		break;
			    		case "broadCastNews":
			    			$('#rotationPhoto').removeAttr("hidden");
			    			$('#coverPhoto').attr("hidden","hidden");
			    			break;
				    	default:
				    		break;
				    }
